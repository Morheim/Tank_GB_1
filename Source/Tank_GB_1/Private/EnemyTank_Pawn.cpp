...
void AEnemyTank_Pawn::OnConstruction(const FTransform& Transform)
{
	...
	GEngine->AddOnScreenDebugMessage(-1, 10, FColor::Red, FString::Printf(TEXT("Enemy Tank Created %d"),Number));
}
...
{
	Destroy();
	GEngine->AddOnScreenDebugMessage(9818, 3, FColor::White,FString::Printf(TEXT("Enemy Tank [Destroyed]")));
...
{
	GEngine->AddOnScreenDebugMessage(83235042, 3, FColor::Orange,FString::Printf(TEXT("Health [Enemy Tank]: %f"), CurrentHealth));
}
