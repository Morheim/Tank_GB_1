	...
	bool bSelfMoved, FVector HitLocation, FVector HitNormal, FVector NormalImpulse, const FHitResult& Hit)
{
	...
	(Targets, GetActorLocation(), GetActorLocation(),
		GetActorRotation().Quaternion(), ECollisionChannel::ECC_Visibility,
		FCollisionShape::MakeSphere(AffectedArea->GetScaledSphereRadius()));
	...
	for (auto& HitTarget : Targets)
	{
		...
		{
			auto Impulse = GetActorForwardVector() * 1000;
